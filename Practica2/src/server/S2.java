...
public class S2 extends Thread {
    ...
    public void setHv(int nub) {
        this.nub = nub;
    }

    public S2(int puerto, String nombServer, double pa, double nub) {
        try {
            this.puerto = puerto;
            this.pa =pa;
            this.nub = nub;
            ...
            try {
                ...
                while (running){
                    ...
                    */
                    ...
                    // System.out.println("Al server le llega "  + received);
                    if (orden.equals("END")){
                        close();
                        running = false;
                        nonStop=false;
    ...
    public void serializaXml(String nombreServer) {
        escGlob = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" + "\n"  + "<!DOCTYPE Server SYSTEM \"S2.dtd\">" + "\n" + "<Server>" + "\n" +  "\t<hpa>" + pa + "</hpa>" + "\n" + "\t<nub>" + nub + "</nub>" + "\n" + "</Server>";
        // String rutaArchivo = "./sources/" + nombreServer + ".xml"; // Cambia esto por la ruta que desees
    }

    public void serializarJSON(String nombreServer) {

        JsonObject jsonObject = new JsonObject();
        jsonObject.addProperty("Presion Atmosférica", pa);
        jsonObject.addProperty("Nubosidad", nub);
